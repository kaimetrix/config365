<#
.SYNOPSIS
    Creates and manages Intune Win32 LOB apps from user-supplied PowerShell scripts via Microsoft Graph API

.DESCRIPTION
    Reads install.ps1 + detection.ps1 + config.json (and optional uninstall.ps1) from one or more
    /apps/custom/{appName}/ directories (baseline repo and/or tenant repo), packages each as a
...
.EXAMPLE
    .\Configure-CustomApps.ps1 `
        -ConfigDirectories "baseline/apps/custom","Tenant-repo/apps/custom" `
        -WhatIfMode -OutputPath "customapps-plan.json"
...
Write-Host "`n##[section]Discovering Custom app packages"
...
if ($packageMap.Count -eq 0) {
    Write-Host "No Custom app packages found in any of the provided directories." -ForegroundColor DarkGray
    if ($OutputPath) {
        @{ Service = 'CustomApps'; WouldCreateCount = 0; WouldUpdateCount = 0; NoChangeCount = 0; ErrorCount = 0; Results = @() } |
...
Write-Host "`n##[section]Processing Custom apps"
...
$globalTmpDir = Join-Path ([System.IO.Path]::GetTempPath()) "CustomApps-$(Get-Random)"
...
try {
    foreach ($pkg in $packageMap.Values) {
        ...
        $intuneAppName = Resolve-IntuneAppDisplayName -Config $config -PackageId $appName -Prefix 'Custom'
        ...
        $notesValue = "installHash:$installHash;detectionHash:$detectionHash;uninstallHash:$uninstallHash;iconHash:$iconHash"

        # Build Win32 app JSON body
        $version       = if ($config.version) { $config.version } else { '' }
        $jAppName      = $intuneAppName | ConvertTo-Json -Compress
        $jDescription  = "Custom Win32 app: $displayName. Version: $version. Managed by CONFIG365." | ConvertTo-Json -Compress
...
"@
            ...
            $lookup   = Find-ExistingWin32AppForConfig -Config $config -PackageId $appName -Prefix 'Custom' -GetExisting ${function:Get-ExistingWin32App}
            ...
            if ($WhatIfMode) {
                ...
                } elseif ($assignChanged -or $nameChanged -or $iconChanged) {
                    $changes = @()
...
Write-Host "`n##[section]Custom Apps Summary"
