<#
.SYNOPSIS
    Hides or shows Exchange Online mailboxes in the Global Address List (GAL) based on Entra group membership.

.DESCRIPTION
    Reads an exchange-gal-hide configuration from the baseline repo and/or tenant repo. For each
    configured rule, the script:
      ...
      3. Calls Set-Mailbox -HiddenFromAddressListsEnabled for each member's mailbox.
...
.PARAMETER BaselineConfigDir
    Path to the directory containing baseline/exchange-gal-hide.json (the maintenance/ folder
    checked out from the baseline repo).

.PARAMETER TenantConfigDir
    Path to the directory containing config/maintenance/exchange-gal-hide.json in the tenant repo.
...
Write-Host "`n##[section]Loading exchange GAL hide configuration"

function Read-GalHideFile {
    param([string]$Dir)
    if (-not $Dir -or -not (Test-Path $Dir)) { return @() }
    $path = Join-Path $Dir 'exchange-gal-hide.json'
    if (-not (Test-Path $path)) {
        Write-Host "  No exchange-gal-hide.json found in: $Dir" -ForegroundColor DarkGray
        return @()
    }
    try {
        $parsed = Get-Content $path -Raw | ConvertFrom-Json
        $rules  = @($parsed.galHideRules)
...
$baselineRules = Read-GalHideFile -Dir $BaselineConfigDir
$tenantRules   = Read-GalHideFile -Dir $TenantConfigDir
...
if ($allRules.Count -eq 0) {
    Write-Host "No GAL hide rules found. Nothing to do." -ForegroundColor DarkGray
    if ($OutputPath) {
        @{ Service = 'ExchangeGalHide'; Timestamp = (Get-Date -Format 'yyyy-MM-dd HH:mm:ss'); WhatIfMode = [bool]$WhatIfMode; Results = @() } |
            ConvertTo-Json -Depth 10 | Out-File -FilePath $OutputPath -Encoding UTF8 -WhatIf:$false
    }
    exit 0
}

Write-Host "Total GAL hide rules to process: $($allRules.Count)"
...
Write-Host "`n##[section]Processing exchange GAL hide rules"
...
foreach ($rule in $allRules) {
    ...
    $hideTarget = [bool]$rule.hidden

    Write-Host "`n--- Rule: $ruleName ---" -ForegroundColor Cyan
    Write-Host "  Action: $(if ($hideTarget) { 'Hide from GAL' } else { 'Show in GAL' })"
    ...
    foreach ($member in $userMembers) {
        ...
        if ($WhatIfMode) {
            Write-Host "    WouldSet: '$identity' → HiddenFromAddressListsEnabled=$hideTarget" -ForegroundColor Yellow
            $applied++
            continue
        }
        ...
        try {
            Set-Mailbox `
                -Identity                        $identity `
                -HiddenFromAddressListsEnabled   $hideTarget `
                -ErrorAction Stop
            $applied++
        } catch {
            Write-Host "    Warning: Failed to set GAL visibility for '$identity': $_" -ForegroundColor Yellow
            $skipped++
        }
    }
...
Write-Host "`n##[section]Exchange GAL Hide Summary"
...
if ($OutputPath) {
    $baselineGalFile = Join-Path $BaselineConfigDir 'exchange-gal-hide.json'
    $summary = @{
        Service    = 'ExchangeGalHide'
        ...
        Results    = @($allResults | ForEach-Object {
            @{
                ...
                Hidden   = $_.Hidden
                Error    = $_.Error
                FilePath = if (Test-Path $baselineGalFile) { $baselineGalFile } else { $null }
            }
        })
    }
